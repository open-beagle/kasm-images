...
if [[ "${DISTRO}" == @(centos|oracle8|rockylinux9|rockylinux8|oracle9|almalinux9|almalinux8) ]]; then
  if [ ! -z "${CHROME_VERSION}" ]; then
    curl -fL -o chrome.rpm https://dl.google.com/linux/chrome/rpm/stable/x86_64/google-chrome-stable-${CHROME_VERSION}.x86_64.rpm
  else
  ...
  curl -fL -o ./linux_signing_key.pub https://dl.google.com/linux/linux_signing_key.pub
