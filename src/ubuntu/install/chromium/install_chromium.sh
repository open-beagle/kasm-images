...
if [[ "${DISTRO}" == @(oracle8|rockylinux9|rockylinux8|oracle9|almalinux9|almalinux8|fedora39|fedora40) ]]; then
  ...
  if [ ${ARCH} = 'amd64' ] ;
  then
    chrome_url="http://mirrors.tuna.tsinghua.edu.cn/ubuntu/pool/universe/c/chromium-browser/"
  else
    chrome_url="http://mirrors.tuna.tsinghua.edu.cn/ubuntu-ports//pool/universe/c/chromium-browser/"
  fi
